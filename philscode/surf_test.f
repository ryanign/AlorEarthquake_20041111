...
c
      ...
      parameter(Ldim=40)
c
      real*8 clm(Ldim+1,Ldim+1,18), slm(Ldim+1,Ldim+1,18),tlm(18),norm
      real*8 lat,lon,dv
c,phv_hrv
      real T(14), c(20), u(20), q(20)
...
c
      call readhrv()
      call phv_hrvt(lat,lon,T,nt,5,c,u,q)
c      call phv_hrvt(lat,lon,T,nt,5,c)
      do i=1,nt
        write(*,*) T(i),c(i),u(i),q(i)
      enddo
      end
c
c
...
c      coefpath='/Users/phil/repos/ffipy/philscode/surfw/ETL-JGR97'
      coefpath='surfw/ETL-JGR97'
      pi = 3.1415927d0
...
c     adapted to use function call to butter, HKT 00/06/05
      ...
      character*1,intent(in):: lhorb
      parameter (ndim=24000)
c      parameter (ndim=240000)
      complex c(ndim),butter
c x() gets padded, so is difficult to declare as an input
      real x(*)
      real,intent(out):: y(*)
