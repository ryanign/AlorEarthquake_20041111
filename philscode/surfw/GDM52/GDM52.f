...
c
      ...
      real ela,elo,sla,slo
      real c(nt),t(nt),f(nt),u(14)
      data t/30,40,50,60,70,80,90,100,125,150,175,200,225,250/
      call read_gdm52()
      ela=-31.6511040      
      elo=-71.7437439 
c      ela=20.
...
c      slo=-118.
      ...
      do i=1,nt
         f(i) = 1./T(i)
      enddo
      call path_gdm52(elo,ela,slo,sla,lora,f,c,u,nt)
      do i=1,nt
        write(*,*) T(i),c(i),u(i)
      enddo
      end
...
c
      ...
      character*80 dispfile(2),dispdir

      data iprtlv/0/

      dispdir = '/Users/phil/repos/ffipy/philscode/surfw/GDM52/'
      dispfile(1)='LOVE_400_100.disp'
      dispfile(2)='RAYL_320_80_32000_8000.disp'

      inquire(file=dispdir(1:46)//dispfile(1),exist=exists)
      if(.not.exists) then
	   write(*,*) 'Love dispersion model file '//dispdir(1:46)//dispfile(1)(1:17)//' does not exist'
	   stop
      endif
c
      inquire(file=dispdir(1:46)//dispfile(2),exist=exists)
      if(.not.exists) then
	   write(*,*) 'Rayleigh dispersion model file '//dispdir(1:46)//dispfile(2)(1:27)//' does not exist'
	   stop 
      endif
c
...
c
      do imdl=1,2
          ios=0
          open(1,file=dispdir(1:46)//dispfile(imdl),iostat=ios)
          do while(ios.eq.0) 
