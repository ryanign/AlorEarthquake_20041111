...
def kiksyn(nt,dt,ib,ic,strike,dip,rak,z,az,p,g,trise,tdur,istype=3,
           velmod=None,set_velmod=True,pad=False,verbose=False,frq4=None):
    """
...
#      read(2,*) nl2,(vp2(l),vs2(l),den2(l),dep2(l),l=1,nl2)
    ...
    if frq4 is None:
        frq4 = -1.*np.ones(4)
    syn  = kiklib.kiksyn(nt1,dt,ib,ic,az,p,g,strike,dip,rak,z,trise,tdur,
                        istype,tqp,tqs,vmod,ns,set_velmod,frq4)
    return syn[0:nt]
...
def point_disp(elon,elat,edep,strk,dip,pot1,pot2,
               rlon,rlat,alp=0.5,utm=False,dmax=-1.):
    """
    Wrapper around point_slip from dcs_subs
    """
    edsp,ndsp,zdsp = dcs_python.point_disp(alp,elon,elat,edep,strk,dip,\
                                           pot1,pot2,rlon,rlat,utm,dmax)
    return edsp,ndsp,zdsp

def fault_disp(elon,elat,edep,strk,dip,lnt,wdt,disl1,disl2,
               rlon,rlat,rdep=None,alp=0.5,utm=False,dpcor=False,dmax=-1.):
    """
    Wrapper around fault_slip from dcs_subs. From the comments therein:
      This subroutine is a wrapper for the Okada routines that calcualte
   elastic deformation due to a dislocation in an elastic half-space. 
   It returns surface displacement at multiple receiver poinst for 
   mutliple faults. 
 
   Input
      alp           Half-space 'material constant', 2 X Poisson's ratio
      elon,elat     Arrays cotnaining coordinates in degrees for fault 
                    centers
      edep          '' depth of fault centers in km
      strk,dip      '' fault strikes and dips (degrees)
      length,wdt    '' lengths and widths of faults, in km
      disl1,disl2   '' dislocations (hanging wall w.r.t. footwall) in 
                    meters in the along strike and up-dip directions, 
                    resp. 
      rlon,rlat     Arrays containing coordinates in degress of points
                    at which surface displacement is calculated.
      rdep          ''       depths at which displacement is calculated
      utm           True for utm coordinates, false for lat,lons
      dstmx         Distance threshold at which contributions to the 
                    surface displacement are ignored
      dpcor         if true, an approximate correction is applied to 
                    fault depth by subtracting the depth of the 
                    observation point (for use with deep seafloor GPS). 
                    Rdep must be set.
      n,m           Dimensins of elat(), rlon(), and similar.
   Output
      edsp,ndsp,zdsp Surface displacement (in meters0 in the east, 
                    north and vertical directions, resp.
    """
    '''
    print 'Input:'
    print 'alp=',alp
    ...
    print 'rlat=',rlat
    '''
    if dpcor and rdep== None:
        print 'Rdep must be set if depths are to be corrected (dpcor=True)'
        sys.exit(-1)
    if not dpcor:
        rdep = np.zeros(len(rlat))
    edsp,ndsp,zdsp = dcs_python.fault_disp(alp,elon,elat,edep,strk,dip,\
                            lnt,wdt,disl1,disl2,rlon,rlat,rdep,utm,dmax,dpcor)
    '''
    print 'Output:'
    print 'edsp=',edsp
    print 'ndsp=',ndsp
